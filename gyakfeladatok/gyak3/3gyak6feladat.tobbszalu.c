...
void* count_even_part(void* arg) {
    ...
    // A számokat a globális változóhoz adja hozzá
...
int main() {
    ...
    for (int i = 0; i < MAX_THREADS; i++) {
        thread_ids[i] = i;
        pthread_create(&threads[i], NULL, count_even_part, &thread_ids[i]);
    }
