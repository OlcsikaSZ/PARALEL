...
// A szálak által végzett prefix számítás
void* erew_prefix_sum(void* arg) {
...
int main() {
    ...
    for (int i = 0; i < MAX_THREADS; i++) {
        thread_ids[i] = i;
        pthread_create(&threads[i], NULL, erew_prefix_sum, &thread_ids[i]);
    }
    ...
    printf("EREW execution time with %d threads: %.6f seconds\n", MAX_THREADS, execution_time);
